# Security Evaluation Experiment Log

This repository records public notes, experiment summaries, and data references for a Kaggle security benchmark competition.

## Scope

The repository is intentionally limited to public-facing material:

- project background and data notes
- experiment summaries and public metrics
- release notes for public artifacts
- sanitized result tables or figures when available

It does not contain credentials, private data, local runtime logs, submission packages, or private process details.

## Repository Layout

- `docs/` - public documentation and release boundaries
- `experiments/` - dated experiment records
- `public_data/` - public data references and schemas
- `results/` - sanitized summaries and figures

## Current Status

Preparation completed on 2026-06-13:

- competition access verified
- official files stored locally, outside git tracking
- local readiness checks completed
- public repository boundary established

First scored public baseline:

- `s01`: public score `12.060`

Latest completed public scores:

- `s02`: public score `7.940`
- `s03`: public score `2.765`
- `s04`: public score `3.050`
- `s05`: public score `3.135`
- `s07`: public score `19.130`
- `s08`: public score `5.760`
- `s09`: public score `8.950`
- `s10`: public score `3.150`
- `s11`: public score `23.750`
- `s13`: public score `15.840`
- `s15`: public score `20.370`
- `s16`: public score `25.460`
- `s17`: public score `24.060`
- `s18`: public score `22.780`
- `s20`: public score `27.900`
- `s21`: public score `29.430`
- `s22`: public score `31.160`
- `s23`: public score `32.710`
- `s26`: public score `32.460`
- `s29`: public score `33.560`
- `s30`: public score `33.940`
- `s31`: public score `36.000`
- `s32`: public score `40.500`
- `s33`: public score `45.000`
- `s36`: public score `45.900`
- `s37`: public score `47.250`
- `s38`: public score `49.500`
- `s40`: public score `54.000`
- `s46`: public score `57.240`
- `s47`: public score `57.600`
- `s48`: public score `57.690`
- `s49`: public score `57.780`
- `s50`: public score `57.960`
- `s66`: public score `22.500`
- `s67`: public score `27.000`
- `s68`: public score `32.400`
- `s69`: public score `37.800`
- `s70`: public score `43.200`
- `s81`: public score `27.000`
- `s86`: public score `27.000`
- `s87`: public score `30.600`
- `s88`: public score `34.200`
- `s89`: public score `36.000`
- `s90`: public score `37.800`
- `s91`: public score `39.600`
- `s92`: public score `41.040`
- `s93`: public score `42.120`
- `s94`: public score `42.840`
- `s95`: public score `43.200`
- `s100`: public score `43.200`
- `s101`: public score `57.700`
- `s102`: public score `56.845`
- `s103`: public score `56.440`
- `s104`: public score `49.095`
- `s105`: public score `55.395`
- `s106`: public score `52.595`
- `s110`: public score `53.625`
- `s111`: public score `55.065`
- `s112`: public score `54.615`
- `s113`: public score `59.810`
- `s114`: public score `60.745`
- `s115`: public score `4.675`
- `s116`: public score `57.980`
- `s117`: public score `60.590`
- `s118`: public score `59.645`
- `s119`: public score `48.690`
- `s120`: public score `50.670`
- `s152`: public score `82.080`
- `s158`: public score `82.170`
- `s159`: public score `81.900`
- `s160`: public score `76.590`
- `s162`: public score `78.570`
- `s163`: public score `78.705`
- `s164`: public score `69.390`
- `s165`: public score `71.280`
- `s166`: public score `72.530`
- `s167`: public score `0.450`
- `s168`: public score `74.475`
- `s169`: public score `74.430`
- `s170`: public score `78.255`
- `s171`: public score `80.100`
- `s172`: public score `80.370`
- `s173`: public score `81.180`
- `s174`: public score `79.830`
- `s175`: public score `72.180`
- `s176`: public score `75.780`
- `s177`: public score `78.885`
- `s178`: public score `82.170`
- `s179`: public score `82.215`
- `s180`: public score `81.450`
- `s181`: public score `84.915`
- `s182`: public score `81.450`
- `s183`: public score `82.350`
- `s184`: public score `82.350`
- `s185`: public score `83.970`
- `s186`: public score `79.470`
- `s187`: public score `77.355`
- `s188`: public score `80.820`
- `s189`: public score `80.460`
- `s190`: public score `84.510`
- `s191`: public score `83.790`
- `s192`: public score `87.525`
- `s193`: public score `84.825`
- `s194`: public score `84.690`
- `s196`: public score `85.050`
- `s197`: public score `78.930`
- `s198`: public score `83.160`
- `s199`: public score `83.070`
- `s200`: public score `79.155`
- `s201`: public score `76.590`
- `s203`: public score `84.915`
- `s204`: public score `85.140`
- `s205`: public score `83.970`
- `s206`: public score `81.270`
- `s208`: public score `84.285`
- `s209`: public score `84.060`
- `s210`: public score `83.655`
- `s211`: public score `85.005`
- `s212`: public score `86.355`
- `s213`: public score `82.215`
- `s214`: public score `83.475`
- `s215`: public score `81.495`
- `s216`: public score `79.470`
- `s217`: public score `68.850`
- `s219`: public score `78.660`
- `s220`: public score `83.790`
- `s221`: public score `82.485`
- `s222`: public score `86.400`
- `s223`: public score `85.635`
- `s224`: public score `85.680`
- `s225`: public score `88.830`
- `s226`: public score `85.905`
- `s227`: public score `81.990`
- `s228`: public score `86.040`
- `s229`: public score `89.235`
- `s230`: public score `81.090`
- `s231`: public score `89.190`
- `s232`: public score `84.195`
- `s233`: public score `88.155`
- `s234`: public score `84.645`
- `s235`: public score `88.110`
- `s236`: public score `87.210`
- `s237`: public score `38.005`
- `s238`: public score `86.715`
- `s239`: public score `86.310`
- `s240`: public score `87.480`
- `s241`: public score `87.165`
- `s242`: public score `83.310`
- `s243`: public score `84.285`
- `s244`: public score `81.585`
- `s245`: public score `77.805`
- `s246`: public score `77.715`
- `s247`: public score `80.910`
- `s253`: public score `83.295`
- `s254`: public score `79.455`
- `s255`: public score `71.865`
- `s256`: public score `78.930`
- `s257`: public score `81.575`
- `s258`: public score `84.420`
- `s259`: public score `86.040`
- `s260`: public score `85.230`
- `s261`: public score `82.925`
- `s262`: public score `40.600`
- `s263`: public score `84.375`
- `s264`: public score `85.995`
- `s265`: public score `83.880`
- `s266`: public score `87.615`
- `s267`: public score `86.945`
- `s268`: submitted; public score pending
- `s269`: submitted; public score pending
- `s270`: submitted; public score pending
- `s271`: submitted; public score pending
- `s272`: submitted; public score pending

Returned with a public score after evaluation error:

- `s248`: public score `3.850`
- `s249`: public score `5.130`
- `s250`: public score `6.410`
- `s251`: public score `6.410`
- `s252`: public score `7.690`

Completed without a reported public score:

- `s06`
- `s12`
- `s14`
- `s19`
- `s24`
- `s25`
- `s27`
- `s28`
- `s34`
- `s35`
- `s39`
- `s41`
- `s42`
- `s43`
- `s44`
- `s45`
- `s56`
- `s57`
- `s58`
- `s59`
- `s60`
- `s61`
- `s62`
- `s63`
- `s64`
- `s65`
- `s71`
- `s72`
- `s73`
- `s74`
- `s75`
- `s76`
- `s77`
- `s78`
- `s79`
- `s80`
- `s82`
- `s83`
- `s84`
- `s85`
- `s96`
- `s97`
- `s98`
- `s99`
- `s107`
- `s108`
- `s109`

Returned without a public score due to evaluation errors:

- `s51`
- `s52`
- `s53`
- `s54`
- `s55`

Completed without a listed public score:

- `s133`
- `s134`
- `s135`
- `s136`
- `s137`
- `s139`
- `s148`
- `s149`
- `s157`
- `s161`
- `s195`
- `s202`
- `s207`
- `s218`

## Data

Competition files are stored in an ignored local directory. Raw downloads and local caches are not committed. Public data notes are maintained in `public_data/`.

## License

Documentation in this repository is released under the MIT License unless a file states otherwise.
